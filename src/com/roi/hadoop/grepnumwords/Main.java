...
public class Main {
	public static void main(String[] args) throws Exception {
		...
		if (args.length <= 2) {
			System.err
					.println("Usage: grepnumwords <in> <out> <term1> <term2>");
			System.exit(2);
		}

		// set up parameters to Mapper and Reducer
		String[] userSearchTerms = new String[args.length - 2];
		for (int i = 2; i < args.length; ++i) {
			userSearchTerms[i - 2] = args[i];
		}
		conf.setStrings("searchTerms", userSearchTerms);
